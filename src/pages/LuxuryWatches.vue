  ...
  <div class="luxury-watches-page">
    ...
    <header class="luxury-watches-header">
      ...
      <input
        ...
        class="search-input"
        aria-label="Tìm kiếm đồng hồ cao cấp"
      />
    </header>
...
import { fetchLuxuryWatches } from '../services/productService'; // Import hàm dịch vụ
...
export default {
  ...
  methods: {
    ...
    async fetchLuxuryWatchesData() {
      try {
        const data = await fetchLuxuryWatches(); // Gọi hàm dịch vụ để lấy dữ liệu
        this.products = data;
        this.filteredProducts = data;
      } catch (error) {
        this.error = 'Không thể tải dữ liệu đồng hồ cao cấp.';
  ...
  mounted() {
    this.fetchLuxuryWatchesData(); // Gọi API khi trang được tải
  },
};
</script>
